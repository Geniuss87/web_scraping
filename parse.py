...
def save_data(mers: list) -> None:
    with open(CSV_FILE, "w") as file:
        writer = csv.writer(file, delimiter=",")
        writer.writerow(["Title", "Description", "Usage", "Price $", "Price Som", "Image"])
        for car in mers:
            writer.writerow([car["title"], car["description"], car["usage"], car["price"], car["price_som"],
                             car["image"]])
